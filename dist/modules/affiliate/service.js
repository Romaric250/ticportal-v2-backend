...
export class AffiliateService {
    static async createCountry(input) {
        ...
        const normalizeRate = (rate, defaultRate) => {
    ...
    static async createRegion(input) {
     ...
     */
    static async updateCountry(countryId, input) {
        ...
        if (input.studentPrice !== undefined)
            updateData.studentPrice = input.studentPrice;
        if (input.platformFee !== undefined)
            updateData.platformFee = input.platformFee;
     ...
     */
    static async updateRegion(regionId, input) {
    ...
    static async updateUserRole(adminId, input) {
        ...
        if (input.newRole === "AFFILIATE" && input.regionId) {
            ...
            await sendRoleChangeEmail(user, input.newRole, {
                regionName: region.name,
                countryName: region.country.name,
                referralCode,
                referralLink
    ...
    static async activateAffiliate(adminId, input) {
    ...
    static async getAffiliateDashboard(userId) {
        ...
        const recentReferralsWithCommissions = await Promise.all(profile.referrals.map(async (r) => {
    ...
    static async generateReferralCode(user, region) {
        let referralCode;
        let isUnique = false;
        let attempts = 0;
     ...
     */
    static async getReferrals(userId, status, page = 1, limit = 20) {
     ...
     */
    static async getCommissions(userId, status, type, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        const whereClause = {
            userId: userId
        };
     ...
     */
    static async getRegionalCoordinatorDashboard(userId) {
        const profile = await db.affiliateProfile.findUnique({
            where: { userId },
            include: {
                region: {
        ...
        const activeAffiliates = affiliatesInRegion.filter((a) => a.status === 'ACTIVE').length;
        ...
        const earnings = profile.commissions.reduce((acc, comm) => {
        ...
        return {
            ...
            earnings,
            affiliates: affiliatesInRegion.map((a) => ({
                id: a.id,
     ...
     */
    static async getNationalCoordinatorDashboard(userId) {
        const profile = await db.affiliateProfile.findUnique({
            where: { userId },
            include: {
                country: {
        ...
        const allAffiliates = profile.country.regions.flatMap((r) => r.affiliates);
        const totalAffiliates = allAffiliates.filter((a) => a.subRole === AffiliateSubRole.AFFILIATE).length;
        const activeAffiliates = allAffiliates.filter((a) => a.subRole === AffiliateSubRole.AFFILIATE && a.status === 'ACTIVE').length;
        const totalRegionalCoordinators = allAffiliates.filter((a) => a.subRole === AffiliateSubRole.REGIONAL_COORDINATOR || a.subRole === AffiliateSubRole.ASSISTANT_REGIONAL_COORDINATOR).length;
        ...
        return {
            ...
            earnings,
            regions: profile.country.regions.map((r) => ({
                id: r.id,
                name: r.name,
                affiliatesCount: r.affiliates.filter((a) => a.subRole === AffiliateSubRole.AFFILIATE).length,
                activeAffiliatesCount: r.affiliates.filter((a) => a.subRole === AffiliateSubRole.AFFILIATE && a.status === 'ACTIVE').length,
                coordinatorsCount: r.affiliates.filter((a) => a.subRole === AffiliateSubRole.REGIONAL_COORDINATOR || a.subRole === AffiliateSubRole.ASSISTANT_REGIONAL_COORDINATOR).length
            }))
        };
    }
    /**
    ...
    static async getAffiliateProfile(userId) {
        ...
        if (countryData) {
            ...
            else if (profile.subRole === AffiliateSubRole.REGIONAL_COORDINATOR ||
                profile.subRole === AffiliateSubRole.ASSISTANT_REGIONAL_COORDINATOR) {
                commissionRate = countryData.regionalCommissionRate ?? 5;
            }
            else if (profile.subRole === AffiliateSubRole.NATIONAL_COORDINATOR ||
     ...
     */
    static async createAffiliateProfile(userId, regionId, subRole = AffiliateSubRole.AFFILIATE, additionalData) {
     ...
     */
    static async regenerateReferralCode(userId) {
        const profile = await db.affiliateProfile.findUnique({
            where: { userId },
            include: {
                region: {
     ...
     */
    static async getAffiliateReferralStats(affiliateIds) {
        ...
        const paidStatuses = [ReferralStatus.PAID, ReferralStatus.ACTIVATED];
        const map = new Map();
     ...
     */
    static async resolveListAffiliateIdFilters(params) {
        const intersections = [];
        const needEarnFilter = params.earnFilter != null ||
        ...
        if (needEarnFilter) {
            ...
            if (params.earnFilter === "no_commission") {
                ...
                intersections.push(all.map((a) => a.id).filter((id) => (byId.get(id) ?? 0) === 0));
            }
            else {
                ...
                const ids = allProfiles
                    ...
                    if (params.earnFilter === "with_commission" && total <= 0)
                        return false;
                    if (params.minEarned != null && total < params.minEarned)
                        return false;
                    if (params.maxEarned != null && total > params.maxEarned)
                        return false;
                    return true;
                });
        ...
        if (params.paymentChannel && params.paymentChannel !== "all") {
            const methods = params.paymentChannel === "manual"
            ...
            const unique = [...new Set(refs.map((r) => r.affiliateId).filter(Boolean))];
    ...
    static async listAffiliates(params) {
        ...
        const distinctReferralsByAffiliate = new Map();
        for (const c of commissionRows) {
            if (!c.affiliateProfileId)
                continue;
        ...
        const affiliatesWithRecalculatedEarnings = await Promise.all(affiliates.map(async (affiliate) => {
            ...
            const studentsWithCommissions = distinctReferralsByAffiliate.get(affiliate.id)?.size ?? 0;
            return {
                ...affiliate,
     ...
     */
    static async suspendAffiliate(affiliateId, reason) {
        const profile = await db.affiliateProfile.findUnique({
     ...
     */
    static async terminateAffiliate(affiliateId, reason) {
        const profile = await db.affiliateProfile.findUnique({
            where: { id: affiliateId },
            include: { user: true, region: { include: { country: true } } }
        });
     ...
     */
    static async deleteAffiliate(affiliateId) {
        const profile = await db.affiliateProfile.findUnique({
            where: { id: affiliateId },
            include: {
                referrals: true,
     ...
     */
    static async unsuspendAffiliate(affiliateId) {
        const profile = await db.affiliateProfile.findUnique({
            where: { id: affiliateId }
        });
        if (!profile) {
    ...
    static async getSystemLedger(params) {
        ...
        const entries = payments.map(payment => {
            ...
            const affiliateCommission = commissions.find((c) => c.type === 'AFFILIATE')?.commissionAmount || 0;
            const regionalCommission = commissions.find((c) => c.type === 'REGIONAL')?.commissionAmount || 0;
            const nationalCommission = commissions.find((c) => c.type === 'NATIONAL')?.commissionAmount || 0;
     ...
     */
    static async updateCommissionTiers(data, updatedBy) {
        // Validate rates
...
AffiliateService.MANUAL_PAYMENT_METHODS = [
    PaymentMethod.BANK_TRANSFER,
    PaymentMethod.CASH,
    PaymentMethod.OTHER,
];
AffiliateService.ONLINE_PAYMENT_METHODS = [
