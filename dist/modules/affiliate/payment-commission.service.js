...
export class PaymentCommissionService {
     ...
     */
    static async confirmPayment(paymentId, verifiedBy) {
        try {
            // Get payment details
     ...
     */
    static async calculateCommissions(referralId, country) {
        try {
            const referral = await db.studentReferral.findUnique({
                where: { id: referralId },
                include: {
            ...
            if (referral.affiliate) {
                ...
                if (referral.affiliate.user && referral.payment.user) {
                    try {
                        await sendCommissionEarnedEmail(referral.affiliate.user.email, referral.affiliate.user.firstName, {
                            commissionAmount: affiliateCommissionAmount,
                ...
                if (regionalCoordinators.length > 0) {
                    ...
                    if (regionalCoordinator) {
                        ...
                        if (regionalCoordinator.user && referral.payment.user) {
                            try {
                                await sendCommissionEarnedEmail(regionalCoordinator.user.email, regionalCoordinator.user.firstName, {
                                    commissionAmount: regionalCommissionAmount,
                                    currency: country.currency,
            ...
            if (nationalCoordinators.length > 0) {
                ...
                if (nationalCoordinator) {
                    ...
                    if (nationalCoordinator.user && referral.payment.user) {
                        try {
                            await sendCommissionEarnedEmail(nationalCoordinator.user.email, nationalCoordinator.user.firstName, {
                                commissionAmount: nationalCommissionAmount,
                                currency: country.currency,
     ...
     */
    static async processFailedCommissions() {
        try {
     ...
     */
    static async activateReferral(referralId) {
        try {
            const referral = await db.studentReferral.findUnique({
                where: { id: referralId }
            });
     ...
     */
    static async createPayoutBatch(commissionIds, createdBy) {
        try {
            // Get all commissions
     ...
     */
    static async completePayoutBatch(batchId) {
        try {
     ...
     */
    static async getApprovedCommissionsForPayout(limit = 100) {
        const commissions = await db.commission.findMany({
            where: {
                status: CommissionStatus.APPROVED,
                payoutBatchId: null
...
//# sourceMappingURL=payment-commission.service.js.map