...
import { sendTeamInviteEmail, sendTeamRoleUpdateEmail, sendTeamRemovalEmail, sendTeamJoinRequestEmail, sendJoinRequestAcceptedEmail, sendJoinRequestRejectedEmail, sendTeamCreatedEmail, } from "../../shared/utils/email";
...
export class TeamService {
    ...
    static async getUserTeams(userId) {
        ...
        const teamsWithUnread = await Promise.all(teamMembers.map(async (tm) => {
     ...
     * Create a new team
     */
    static async createTeam(userId, input) {
     ...
     * Update team details
     */
    static async updateTeam(userId, teamId, input) {
     ...
     */
    static async addTeamMember(inviterId, teamId, input) {
        ...
        await sendTeamInviteEmail(user.email, user.firstName, inviter.team.name, `${inviter.user.firstName} ${inviter.user.lastName}`);
     ...
     */
    static async updateTeamMemberRole(userId, teamId, memberId, input) {
        // Verify user is team lead
        ...
        // Send email notification
        await sendTeamRoleUpdateEmail(teamMember.user.email, teamMember.user.firstName, teamMember.team.name, input.role);
     ...
     */
    static async removeTeamMember(userId, teamId, memberId) {
        ...
        if (!isSelf) {
            await sendTeamRemovalEmail(memberToRemove.user.email, memberToRemove.user.firstName, memberToRemove.team.name);
     ...
     */
    static async getTeamChats(userId, teamId, page = 1, limit = 50) {
     ...
     */
    static async sendTeamChatMessage(teamId, userId, input) {
        // Verify user is a team member
        const member = await db.teamMember.findFirst({
            where: {
                teamId,
     ...
     */
    static async getUnreadMessageCount(teamId, userId) {
        // Verify user is a team member
     ...
     */
    static async markMessagesAsRead(teamId, userId, messageIds) {
        // Verify user is a team member
        const isMember = await TeamService.isTeamMember(teamId, userId);
        if (!isMember) {
            throw new Error("You are not a member of this team");
        ...
        logger.info({ userId, teamId, markedCount: unreadMessages.length }, "Messages marked as read");
        return { markedCount: unreadMessages.length };
    }
    /**
     * Get unread counts for all user's teams
     */
    static async getUnreadCountsForUserTeams(userId) {
     ...
     */
    static async searchTeams(input) {
     ...
     */
    static async requestToJoinTeam(userId, teamId, input) {
        ...
        await Promise.all(teamLeads.map(async (lead) => {
            // Send email
            await sendTeamJoinRequestEmail(lead.user.email, lead.user.firstName, team.name, `${user.firstName} ${user.lastName}`, input.message);
            // Send notification
            await NotificationService.notifyTeamJoinRequest(lead.userId, `${user.firstName} ${user.lastName}`, team.name, teamId, joinRequest.id);
        }));
        // Track activity
        await activityService.trackActivity({
            userId,
     ...
     */
    static async handleJoinRequest(userId, teamId, requestId, input) {
        // Verify user is team lead
        const userMember = await db.teamMember.findFirst({
            where: {
                teamId,
        ...
        if (input.action === "accept") {
            ...
            await sendJoinRequestAcceptedEmail(joinRequest.user.email, joinRequest.user.firstName, joinRequest.team.name, input.message);
            ...
            await sendJoinRequestRejectedEmail(joinRequest.user.email, joinRequest.user.firstName, joinRequest.team.name, input.message);
     ...
     */
    static async getTeamJoinRequests(userId, teamId) {
        // Verify user is team lead
        const userMember = await db.teamMember.findFirst({
            where: {
                teamId,
