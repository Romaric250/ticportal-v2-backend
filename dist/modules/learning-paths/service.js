...
export class LearningPathService {
     ...
     */
    static async getAllPaths(filters) {
        const where = {};
        if (filters?.audience) {
     ...
     */
    static async getPathById(pathId) {
     ...
     */
    static async getPathsForUser(userId, userRole) {
        // Determine audience based on role
        const audiences = [LearningPathAudience.EVERYONE];
        if (userRole === "STUDENT") {
     ...
     */
    static async updateUserProgress(userId, pathId) {
     ...
     */
    static async updateTotalModules(pathId) {
        const totalModules = await db.learningModule.count({
            where: { pathId },
        });
     ...
     */
    static async enrollUser(userId, pathId) {
        const path = await db.learningPath.findUnique({
            where: { id: pathId },
        });
        if (!path) {
     ...
     */
    static async autoEnrollStudents(pathId) {
     ...
     */
    static async checkPathCompletion(userId, pathId) {
     ...
     */
    static async getModuleStatus(userId, moduleId) {
     ...
     */
    static async getPathModulesWithStatus(userId, pathId) {
     ...
     */
    static async completeLearningPath(userId, pathId) {
        // Check if learning path exists
        const path = await db.learningPath.findUnique({
            where: { id: pathId },
            include: {
     ...
     */
    static async unenrollFromPath(userId, pathId) {
     ...
     */
    static async getAllEnrollmentStatus(userId) {
        // Get all learning paths
        ...
        const enrollmentMap = new Map(enrollments.map(e => [e.learningPathId, e]));
        const completionMap = new Map(completions.map(c => [c.learningPathId, c]));
