...
export class LearningPathController {
     ...
     */
    static async getPathsForUser(req, res) {
        try {
     ...
     */
    static async enrollInPath(req, res) {
        try {
            ...
            const statusCode = error.message === "Learning path not found"
                ? 404
    ...
    static async submitQuiz(req, res) {
        try {
            ...
            const statusCode = error.message === "Module not found"
                ? 404
     ...
     */
    static async getModuleStatus(req, res) {
        try {
            const { moduleId } = req.params;
            const userId = req.user?.userId;
            if (!userId) {
     ...
     *         description: Learning path not found
     */
    static async getPathModules(req, res) {
        try {
            const { pathId } = req.params;
            const userId = req.user?.userId;
            if (!userId) {
     ...
     *         description: Learning path not found
     */
    static async completeLearningPath(req, res) {
        try {
            const { pathId } = req.params;
            const userId = req.user?.userId;
            if (!userId) {
            ...
            const statusCode = error.message === "Learning path not found" ? 404 :
     ...
     */
    static async unenrollFromPath(req, res) {
        try {
            const { pathId } = req.params;
            const userId = req.user?.userId;
            if (!userId) {
     ...
     *                         format: date-time
     */
    static async getAllEnrollmentStatus(req, res) {
        try {
            const userId = req.user?.userId;
            if (!userId) {
                return res.status(401).json({
     ...
     *                         type: boolean
     */
    static async calculateProgress(req, res) {
        try {
            const userId = req.user?.userId;
            if (!userId) {
                return res.status(401).json({
