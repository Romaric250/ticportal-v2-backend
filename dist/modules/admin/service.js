...
import { sendTicCommunityWelcomeEmail } from "../../shared/utils/email.js";
export class AdminService {
     ...
     */
    static async teamIdsWithAtLeastNRequiredSubmitted(min) {
    ...
    static async getTeams(filters) {
        ...
        const minDel = filters.minDeliverablesSubmitted != null && Number.isFinite(Number(filters.minDeliverablesSubmitted))
        ...
        const hasNarrowWhere = Boolean(filters.search?.trim() || filters.school || filters.region?.trim()) || minDel >= 1;
        const teamSelect = {
            id: true,
            name: true,
            school: true,
        ...
        let deliverableTotalByTeam = new Map();
        let deliverableSubmittedByTeam = new Map();
    ...
    static async getDistinctTeamSchools() {
        const rows = await db.team.findMany({
            select: { school: true },
            distinct: ["school"],
            orderBy: { school: "asc" },
        });
        return rows.map((r) => r.school?.trim()).filter((s) => Boolean(s));
    }
    /**
     * Get team by ID
     */
    ...
    static async adminCreateTeam(input) {
        ...
        for (const uid of uniqueMembers) {
            ...
            if (!u)
                throw new Error(`Member user ${uid} not found`);
            const m = await db.teamMember.findFirst({
                where: { userId: uid },
                select: { teamId: true, team: { select: { name: true } } },
            });
            if (m)
                throw new Error(`User ${uid} is already a member of team "${m.team.name}"`);
        }
    ...
    static async sendTicCommunityWelcomeToUser(userId) {
        const user = await db.user.findUnique({
            where: { id: userId },
