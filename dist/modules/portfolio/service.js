...
export class PortfolioService {
    ...
    static async getPortfolioOverview(userId) {
        try {
            ...
            const [profile, journey, project, certs, badges, feedback, skills] = await Promise.all([
            ...
            logger.error({ error: error.message, userId }, "Failed to get portfolio overview");
            throw error;
        }
    }
    /**
    ...
    static async getProfileInfo(userId) {
        ...
        const userRankIndex = allUserPoints.findIndex((up) => up.userId === userId);
     ...
     */
    static async getHackathonJourney(userId) {
        try {
            const phases = [];
            ...
            if (teamMember) {
                const teamMembers = teamMember.team.members.map((m) => ({
                    id: m.user.id,
     ...
     */
    static async getFeaturedProject(userId) {
        try {
            // For now, return null
     ...
     * Get mentor feedback
     */
    static async getMentorFeedback(userId) {
        try {
     ...
     */
    static calculateLevelFromPoints(points) {
        const levels = [
