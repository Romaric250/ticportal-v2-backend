...
import { notifyCommunityPush } from "./push.js";
...
export class CommunityService {
    ...
    static async listRootMessages(limit = 80) {
    ...
    static validatePayload(body, imageUrls) {
        const t = body?.trim() ?? "";
    ...
    static async createMessage(userId, userRole, input) {
    ...
    static async setPinned(adminUserId, adminRole, messageId, pinned) {
    ...
    static async updateMessage(userId, messageId, input) {
        const msg = await db.communityMessage.findFirst({
    ...
    static async deleteMessage(userId, userRole, messageId) {
        const msg = await db.communityMessage.findFirst({
            where: { id: messageId, channelId: CHANNEL },
        });
        if (!msg) {
     ...
     */
    static async bulkDeleteMessages(adminRole, ids) {
        if (!CommunityService.isAdminRole(adminRole)) {
    ...
    static async savePushSubscription(userId, sub) {
    ...
    static async removePushSubscription(userId, endpoint) {
