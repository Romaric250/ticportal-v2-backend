 ...
 */
export const registerTeamChatHandlers = (io, socket) => {
    /**
     * Join a team room
     */
    socket.on("team:join", async ({ teamId }) => {
        try {
            logger.info({ userId: socket.userId, teamId, socketId: socket.id, userName: socket.user?.fullName }, "🔵 [SOCKET] User attempting to join team room");
            if (!socket.userId) {
            ...
            if (!isMember) {
                logger.warn({ userId: socket.userId, teamId, socketId: socket.id }, "❌ [SOCKET] Join failed: Not a team member");
            ...
            logger.info({ userId: socket.userId, teamId, userName: socket.user?.fullName, payload: onlineStatusPayload }, "📢 [SOCKET] Broadcasting online status to team");
        }
        catch (error) {
    ...
    socket.on("team:leave", async ({ teamId }) => {
        try {
            logger.info({ userId: socket.userId, teamId, socketId: socket.id, userName: socket.user?.fullName }, "🔵 [SOCKET] User attempting to leave team room");
            if (!socket.userId)
                return;
            // Leave the team room
            await socket.leave(`team:${teamId}`);
            logger.info({ userId: socket.userId, teamId, userName: socket.user?.fullName }, "✅ [SOCKET] User successfully left team room");
    ...
    socket.on("team:message:send", async ({ teamId, message, attachments }) => {
        try {
            ...
            const isMember = await TeamService.isTeamMember(teamId, socket.userId);
            if (!isMember) {
                logger.warn({ userId: socket.userId, teamId, userName: socket.user.fullName }, "❌ [SOCKET] Send message failed: Not a team member");
                socket.emit("error", { message: "Not a team member" });
                return;
            }
            logger.info({ userId: socket.userId, teamId, userName: socket.user.fullName }, "💾 [SOCKET] Saving message to database");
            // Save message to database
            const chatMessage = await TeamService.sendTeamChatMessage(teamId, socket.userId, {
                message,
                attachments,
            });
            logger.info({ userId: socket.userId, teamId, messageId: chatMessage.id, userName: socket.user.fullName }, "✅ [SOCKET] Message saved to database");
    ...
    socket.on("team:typing:start", async ({ teamId }) => {
        try {
            logger.info({ userId: socket.userId, teamId, userName: socket.user?.fullName }, "⌨️ [SOCKET] User started typing");
            if (!socket.userId || !socket.user)
                return;
            // Verify user is a team member
            const isMember = await TeamService.isTeamMember(teamId, socket.userId);
            ...
            logger.info({ userId: socket.userId, teamId, userName: socket.user.fullName }, "📢 [SOCKET] Broadcasting typing indicator (start) to team");
        }
        catch (error) {
    ...
    socket.on("team:typing:stop", async ({ teamId }) => {
        try {
            logger.info({ userId: socket.userId, teamId, userName: socket.user?.fullName }, "⌨️ [SOCKET] User stopped typing");
            if (!socket.userId || !socket.user)
                return;
            // Verify user is a team member
            const isMember = await TeamService.isTeamMember(teamId, socket.userId);
            ...
            socket.to(`team:${teamId}`).emit("team:typing", typingPayload);
            logger.info({ userId: socket.userId, teamId, userName: socket.user.fullName }, "📢 [SOCKET] Broadcasting typing indicator (stop) to team");
        }
        catch (error) {
            logger.error({ error, teamId, userId: socket.userId }, "💥 [SOCKET] Error broadcasting typing indicator");
        }
    ...
    socket.on("team:message:delivered", async ({ messageId, teamId }) => {
        try {
            logger.info({ userId: socket.userId, teamId, messageId, userName: socket.user?.fullName }, "✉️ [SOCKET] Message delivered receipt");
            if (!socket.userId)
                return;
            ...
            logger.info({ userId: socket.userId, teamId, messageId }, "📢 [SOCKET] Broadcasting delivery receipt to team");
        }
        catch (error) {
    ...
    socket.on("team:message:read", async ({ messageId, teamId }) => {
        try {
            logger.info({ userId: socket.userId, teamId, messageId, userName: socket.user?.fullName }, "👁️ [SOCKET] Message read receipt");
            if (!socket.userId)
                return;
            ...
            socket.to(`team:${teamId}`).emit("team:message:receipt", receiptPayload);
            logger.info({ userId: socket.userId, teamId, messageId }, "📢 [SOCKET] Broadcasting read receipt to team");
        }
        catch (error) {
    ...
    socket.on("disconnect", async () => {
        try {
            logger.info({ userId: socket.userId, socketId: socket.id, userName: socket.user?.fullName }, "🔴 [SOCKET] User disconnected");
            if (!socket.userId)
                return;
            // Get all team rooms the user was in
            const rooms = Array.from(socket.rooms).filter((room) => room.startsWith("team:"));
            logger.info({ userId: socket.userId, userName: socket.user?.fullName, roomsCount: rooms.length, rooms }, "🔍 [SOCKET] Found team rooms user was in");
            ...
            for (const room of rooms) {
                ...
                logger.info({ userId: socket.userId, teamId, userName: socket.user?.fullName, room }, "📢 [SOCKET] Broadcasting offline status to team on disconnect");
            }
            logger.info({ userId: socket.userId, socketId: socket.id, userName: socket.user?.fullName }, "✅ [SOCKET] User disconnect handled successfully");
        }
        catch (error) {
 ...
 */
export const emitTeamUpdate = (io, teamId, data) => {
    const payload = {
        teamId,
        ...data,
    };
    io.to(`team:${teamId}`).emit("team:updated", payload);
    logger.info({ teamId, payload }, "📢 [SOCKET EMIT] Broadcasting team update to all members");
};
/**
 * Emit team member added event
 */
export const emitTeamMemberAdded = (io, teamId, data) => {
    const payload = {
        teamId,
        ...data,
    };
    io.to(`team:${teamId}`).emit("team:member:added", payload);
    logger.info({ teamId, userId: data.userId, userName: data.userName, role: data.role, payload }, "📢 [SOCKET EMIT] Broadcasting member added to all team members");
};
/**
 * Emit team member removed event
 */
export const emitTeamMemberRemoved = (io, teamId, data) => {
    const payload = {
        teamId,
        ...data,
    };
    io.to(`team:${teamId}`).emit("team:member:removed", payload);
    logger.info({ teamId, userId: data.userId, userName: data.userName, payload }, "📢 [SOCKET EMIT] Broadcasting member removed to all team members");
};
/**
 * Emit team member role updated event
 */
export const emitTeamMemberRoleUpdated = (io, teamId, data) => {
    const payload = {
        teamId,
        ...data,
    };
    io.to(`team:${teamId}`).emit("team:member:role:updated", payload);
    logger.info({ teamId, userId: data.userId, userName: data.userName, newRole: data.newRole, payload }, "📢 [SOCKET EMIT] Broadcasting role update to all team members");
};
