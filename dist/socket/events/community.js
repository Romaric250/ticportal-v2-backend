 ...
 */
export const registerCommunityHandlers = (io, socket) => {
    ...
    socket.on("community:typing", (payload) => {
        try {
            if (!socket.userId || !socket.user)
                return;
            const parentId = payload?.parentId === undefined || payload?.parentId === null
                ? null
    ...
    socket.on("community:message:send", async (payload) => {
        try {
            if (!socket.userId || !socket.user) {
                socket.emit("error", { message: "Not authenticated" });
                return;
