...
export const sendTicCommunityWelcomeEmail = async (email, firstName) => {
  ...
  `;
    await sendEmail(email, "Welcome to TIC Community — TIC Portal", emailTemplate(content));
};
...
// Email: Badge Earned
export const sendBadgeEarnedEmail = async (email, firstName, badgeDetails) => {
    const content = `
    <h2>Badge Earned</h2>
    <p>Hello ${firstName},</p>
  ...
  `;
    await sendEmail(email, `Badge Earned: ${badgeDetails.badgeName} - TIC Portal`, emailTemplate(content));
};
// Email: Commission Earned
export const sendCommissionEarnedEmail = async (email, firstName, commissionDetails) => {
    const formattedAmount = new Intl.NumberFormat('en-US', {
        style: 'currency',
  ...
  `;
    await sendEmail(email, `Commission Earned - TIC Portal`, emailTemplate(content));
};
