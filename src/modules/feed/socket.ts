...
export class FeedSocketEmitter {
   ...
   */
  static async emitPointsEarned(userId: string, points: number, reason: string, metadata?: any) {
    try {
      const feedNamespace = this.io.of("/feed");
   ...
   */
  static async emitPointsSummaryUpdate(userId: string, totalPoints: number) {
    try {
      const feedNamespace = this.io.of("/feed");
