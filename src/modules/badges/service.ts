...
export class BadgeService {
  ...
  private static async checkBadgeRequirement(
    userId: string,
    badge: Badge,
    stats: Awaited<ReturnType<typeof BadgeService.getUserStats>>
  ): Promise<boolean> {
    ...
    switch (type) {
      ...
      case "EARLY_POST":
        ...
        if (!user || !firstPost) return false;
    ...
    try {
      ...
      for (const badge of ALL_BADGES) {
        ...
        const percentage = Math.min(
          100,
          (currentValue / badge.requirement.value) * 100
        );
      ...
      return progress.sort((a, b) => {
        ...
        if (!a.earned && b.earned) return -1;
        if (!a.earned && !b.earned) return b.percentage - a.percentage;
   ...
   */
  static async getBadgeAwardCount(badgeId: string): Promise<number> {
    try {
   ...
   */
  static async checkBadgesForAllUsers(): Promise<number> {
    try {
      ...
      for (const user of users) {
        try {
          ...
          if (newBadges.length > 0) {
            logger.info(
              { userId: user.id, badges: newBadges },
              "New badges awarded to user"
            );
          }
        } catch (error) {
      ...
      logger.info(
        { totalUsers: users.length, totalBadgesAwarded },
        "Badge check completed for all users"
      );
   ...
   */
  static async updateBadge(badgeId: string, updates: any): Promise<any> {
    try {
