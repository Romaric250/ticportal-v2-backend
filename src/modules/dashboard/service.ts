...
export class DashboardService {
   ...
   */
  static async getDashboardOverview(userId: string): Promise<DashboardOverview> {
    try {
  ...
  private static calculateLevel(points: number) {
    ...
    const percentage = nextLevelInfo
      ? Math.min(
          100,
          ((points - currentLevelInfo.minPoints) /
   ...
   */
  private static async getNextModule(
    userId: string
  ): Promise<NextModule | null> {
    try {
      ...
      if (enrollments.length === 0) {
        ...
        return {
          ...
          description:
            firstModule.content?.substring(0, 200) ||
      ...
      for (const enrollment of enrollments) {
        ...
        if (completedCount >= path.modules.length) continue;
        ...
        return {
          ...
          description:
            nextModule.content?.substring(0, 200) ||
   ...
   * Get user's team
   */
  private static async getUserTeam(userId: string): Promise<TeamInfo | null> {
    try {
    ...
    try {
      ...
      if (teamIds.length > 0) {
        ...
        for (const template of deliverableTemplates) {
          ...
          let subtitle = "";
          let variant: "danger" | "info" | "purple" = "info";
   ...
   * Get recent badges
   */
  private static async getRecentBadges(
    userId: string,
    limit: number = 3
  ): Promise<RecentBadge[]> {
    try {
   ...
   * Get badge stats
   */
  private static async getBadgeStats(userId: string) {
    try {
      ...
      const totalPoints = badgePoints.reduce(
