...
import { checkGDriveAccess, isGoogleDriveUrl, bulkCheckGDriveAccess, type GDriveCheckResult } from "./gdrive-checker";
...
export class DeliverableService {
   ...
   */
  static async checkDeliverableAccess(deliverableId: string): Promise<{
    deliverableId: string;
  ...
  static async bulkCheckDeliverableAccess(filters?: {
    ...
    const gdriveItems = deliverables.filter(
      (d) => d.contentType === "URL" && isGoogleDriveUrl(d.content),
    );

    const accessMap = await bulkCheckGDriveAccess(
      gdriveItems.map((d) => ({ id: d.id, url: d.content })),
    );
    ...
    const items = gdriveItems.map((d) => {
      ...
      if (accessResult.accessible === true) accessible++;
      else if (accessResult.accessible === false) notAccessible++;
      else checkFailed++;
   ...
   */
  static async rejectDeliverableForAccess(
    deliverableId: string,
    reviewerId?: string,
  ) {
    const deliverable = await db.teamDeliverable.findUnique({
    ...
    let validReviewerId: string | null = null;
    if (reviewerId && reviewerId.length === 24) validReviewerId = reviewerId;

    const reason =
      "Your Google Drive link is not publicly accessible. Please update the sharing settings to 'Anyone with the link' and re-submit.";
    ...
    for (const m of members) {
      ...
      try {
        ...
        await sendEmail(
          m.user.email,
          `Action Required: "${templateTitle}" — Access Issue`,
          wrapEmailTemplate(emailContent),
        );
      } catch (e: any) {
