 ...
 */
export function extractGDriveId(url: string): { id: string | null; isFolder: boolean } {
 ...
 */
export async function checkGDriveAccess(url: string, apiKey?: string): Promise<GDriveCheckResult> {
 ...
 */
async function checkGDriveAccessNoKey(
  id: string,
  isFolder: boolean,
  originalUrl: string,
): Promise<GDriveCheckResult> {
  try {
 ...
 */
export async function bulkCheckGDriveAccess(
  items: { id: string; url: string }[],
  concurrency = 5,
): Promise<Map<string, GDriveCheckResult>> {
  const results = new Map<string, GDriveCheckResult>();
  const queue = [...items];
