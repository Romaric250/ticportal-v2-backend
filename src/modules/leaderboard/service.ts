...
export class LeaderboardService {
  static async getUserRank(userId: string): Promise<UserRankStats> {
    try {
  ...
  static async getStudentsLeaderboard(query: LeaderboardQuery) {
    try {
      ...
      if (minTP !== undefined || maxTP !== undefined) {
        filteredUserPoints = userPoints.filter((up) => {
          ...
          if (minTP !== undefined && tp < minTP) return false;
          if (maxTP !== undefined && tp > maxTP) return false;
          return true;
        });
      }
      ...
      const matched: { user: (typeof users)[0]; tp: number }[] = [];
      for (const up of filteredUserPoints) {
        const user = userMap.get(up.userId);
        if (user) matched.push({ user, tp: tpMap.get(up.userId) || 0 });
      }
      ...
      const leaderboard: StudentLeaderboardEntry[] = pageSlice.map((m, i) => {
  ...
  static async getTeamsLeaderboard(query: LeaderboardQuery) {
    try {
      ...
      if (allMemberIds.length > 0) {
        ...
        for (const a of aggs) pointsByUser.set(a.userId, a._sum.amount || 0);
      }
      ...
      const leaderboard: TeamLeaderboardEntry[] = pageSlice.map((ts, i) => ({
        id: ts.team.id,
        teamId: ts.team.id,
        rank: skip + i + 1,
        name: ts.team.name,
  ...
  static async getSchoolsLeaderboard(query: LeaderboardQuery) {
    try {
      ...
      const studentIdsBySchool = new Map<string, string[]>();
      ...
      const teamCountBySchool = new Map<string, number>();
      if (schoolNames.length > 0) {
        ...
        for (const g of teamGroups) teamCountBySchool.set(g.school, g._count.id);
      }
      ...
      const leaderboard: SchoolLeaderboardEntry[] = pageSlice.map((school, i) => ({
        id: school.name,
        rank: skip + i + 1,
        name: school.name,
   ...
   */
  private static async batchActivityTrends(userIds: string[]): Promise<Map<string, number>> {
  ...
  private static generateInitials(firstName?: string | null, lastName?: string | null): string {
    ...
    if (first && first.length >= 2) return first.substring(0, 2).toUpperCase();
    if (first) return first.toUpperCase();
    if (last && last.length >= 2) return last.substring(0, 2).toUpperCase();
