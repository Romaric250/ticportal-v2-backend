...
export class LeaderboardService {
  ...
  static async getTeamsLeaderboard(query: LeaderboardQuery) {
    try {
      ...
      const leaderboard: TeamLeaderboardEntry[] = teamScores.map((ts) => {
  ...
  static async getSchoolsLeaderboard(query: LeaderboardQuery) {
    try {
      ...
      const schoolScores = await Promise.all(
        schools.map(async (school) => {
          if (!school.school) return null;
          ...
          // Calculate rank change
          const rankChange = await this.calculateSchoolRankChange(
            school.school,
            totalTP
          );
      ...
      const validSchools = schoolScores.filter((s) => s !== null) as any[];
      ...
      const leaderboard: SchoolLeaderboardEntry[] = validSchools.map((school) => {
   ...
   */
  private static async calculateActivityTrend(userId: string): Promise<number> {
    try {
      const now = new Date();
   ...
   */
  private static async calculateTeamActivityTrend(
    memberIds: string[]
  ): Promise<number> {
    try {
      const now = new Date();
   ...
   */
  private static async calculateTeamRankChange(
    teamId: string,
    currentTP: number
  ): Promise<number> {
   ...
   */
  private static async calculateSchoolActivityTrend(
    studentIds: string[]
  ): Promise<number> {
    try {
      const now = new Date();
   ...
   */
  private static async calculateSchoolRankChange(
    schoolName: string,
    currentTP: number
  ): Promise<number> {
    // Simplified implementation - can be enhanced
