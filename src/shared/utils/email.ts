...
export const sendRoleChangeEmail = async (
  ...
  await sendEmail(
    user.email,
    `Your Role Has Been Updated - TIC Portal`,
    emailTemplate(content)
  );
};

function getRoleDetails(role: string, metadata: any) {
  switch (role) {
    case "AFFILIATE":
      return {
        message: `
...
export const sendAffiliateActivationEmail = async (
  user: any,
  profile: any
) => {
  const content = `
